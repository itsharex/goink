...
//
// (story_arc_id, sequence) 联合唯一约束。同弧线内按 sequence 升序排列即为完整演进链。
// target_chapter 是 LLM 对不确定未来的估算，只排不滤——不准确时 review agent 校准。
// actual_chapter 记录实际发生在哪章，0=未发生。
type ArcNode struct {
	ID            int64     `gorm:"column:id;primaryKey;autoIncrement" json:"id"`
	NovelID       int64     `gorm:"column:novel_id;not null;index"                                            json:"novel_id"`
	StoryArcID    int64     `gorm:"column:story_arc_id;uniqueIndex:uk_arc_sequence;not null;index"            json:"story_arc_id"`
	Sequence      int       `gorm:"column:sequence;uniqueIndex:uk_arc_sequence;not null"                      json:"sequence"`       // 弧线内排序 1,2,3...
